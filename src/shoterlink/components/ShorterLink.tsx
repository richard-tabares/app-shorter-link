...
export const ShorterLink = () => {
	...
	const { inputUrl, onInputChange, onResetForm } = useForm(initialState)
		...
		<section className='bg-lightBg rounded-lg p-3 w-full justify-center shadow border border-subtitle/15'>
			...
			<button
				className={`btn-primary ${
					loading && 'bg-accentBlue! opacity-70 cursor-not-allowed'
				}`}
				onClick={() => createShortLink(inputUrl)}
				disabled={loading}>
				{loading ? 'Acortando' : 'Acortar Link'} {loading && <Loading />}
			</button>
